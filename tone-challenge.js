...
(function () {
  ...
  function updateVad(rms, now, hz) {
    ...
    if (!inSpeech) {
      ...
      if (voiced) {
        speechRunFrames++;
        if (speechRunFrames >= cfg('SPEECH_START_FRAMES', 3)) { inSpeech = true; everVoiced = true; return 'speech'; }
  ...
  function isRealUtterance() {
    ...
    if (voiced.length < cfg('MIN_VOICED_FRAMES', 6)) return false;
    var span = voiced[voiced.length - 1].t - voiced[0].t;
    return span >= cfg('MIN_SPEECH_MS', 200);
  }
