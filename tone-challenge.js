...
(function () {
  ...
  function wireOnce() {
    ...
    // The in-game "Voice: …" pill returns to setup (like the Trainer's switch).
    var sw = $('tc-profile-switch');
    if (sw) sw.addEventListener('click', backToSetup);

    // How-to accordion(s) in the setup screen. The trainer's wireHowtoAccordion
    // only covers #view-tone, so we wire our own here. Default OPEN; tapping the
    // header toggles it. (No persistence needed for a single intro section.)
    var view = $('view-tone-challenge');
    if (view) {
      view.querySelectorAll('.tone-howto-section').forEach(function (section) {
        section.classList.add('open'); // default expanded on first paint
  ...
  function backToSetup() {
    if (typeof window.teardownToneChallenge === 'function') window.teardownToneChallenge();
    setMode('setup');
    renderProfileList();
    renderCountGrid();
  }
  ...
  function updateVad(rms, now) {
    ...
    if (!loud && (now - lastLoudTime) > cfg('SPEECH_END_MS', 180)) { inSpeech = false; return 'ended'; }
    return 'speech';
  }
  ...
  function scoreAttempt(core, centreHz) {
    ...
    if (pct >= 75) { if (typeof playSound === 'function') playSound('snd-bingo-correct'); if (typeof haptic === 'function') haptic(25); }
    else if (pct < 40) { if (typeof playSound === 'function') playSound('snd-sentence-fail'); if (typeof haptic === 'function') haptic([20, 40]); }
    else { if (typeof haptic === 'function') haptic(15); }
  ...
  function renderWord(w) {
    ...
    // English meaning + explicit tone name (so non-readers know the target).
    var lbl = TONE_LABEL[w.tone] || { en: w.tone, th: '' };
    if (en) {
      en.innerHTML = escapeHtml(w.en) +
        ' <span class="tc-word-tone">\u00B7 ' + escapeHtml(lbl.en) + ' tone' +
        (lbl.th ? ' <span class="th">' + escapeHtml(lbl.th) + '</span>' : '') + '</span>';
    }
  }
  ...
  function startChallenge() {
    ...
    var st = window.state;
    if (st) { st.toneLastProfileId = active.id; persist(); }
  ...
  function showEndModal(scorePct, passed, rating, tierCounts, trouble, wordCount, unlockedBadge) {
    ...
    if (title) title.textContent = 'Challenge complete';
    if (typeof setEndThaiLine === 'function') setEndThaiLine('win-title-thai', passed ? 'win' : 'lose');
