...
(function () {
  ...
  const SPEECH_START_FRAMES = 3;     // consecutive VOICED+loud frames needed to confirm onset.
                                     // Raised from 2 and now requires PITCH (see updateVad):
                                     // a click/breath/tap is loud but unvoiced, so it can no
                                     // longer fake a word onset and end the capture early.
  ...
  const MIN_SPEECH_MS = 200;         // a finalized word must contain at least this much real
                                     // voiced speech; a shorter segment is treated as a false
                                     // start (a stray click) — discarded, and we keep listening
                                     // instead of finalizing and erroring out.
  const MIN_VOICED_FRAMES = 6;       // ...and at least this many voiced pitch frames collected.
  ...
  function computeToneScores(core, centreHz) {
    ...
    if (net >= 1.5) {
      ...
      // FALLING contour (which keeps dropping) cannot trigger it.
  ...
  function stopCalibration(cancelled) {
    ...
    // discard the earlier good words).
    ...
    // More words to go? Advance and prompt for the next one.
    if (calWordIdx < CAL_WORD_COUNT - 1) {
      calWordIdx++;
      renderCalWord();
    ...
    // That was the final word: finalize from the per-word medians.
    finalizeCalibration();
  }
